...
class FolderItem:
    ...
    def UnselectAll():
        projectWindowEditor = GlobalVars.Particle.screenOrganization.windowEditors.get("Project")
    ...
    def Draw(listOfItems,frame,projectWindowEditor):
        ...
        if columns <= 0:
            columns = 1
        count = 0
        rm = [GlobalVars.Particle.config.get("projectPath"),os.path.join(GlobalVars.Particle.config.get("projectPath"),"Assets")]
...
class ProjectWindowEditor(WindowEditor):
    ...
    def __init__(self, master=None):
        ...
        #FolderView
        self.currentFolder = os.path.join(self.Particle.config.get("projectPath"), "Assets")
        self.Items = []
    ...
    def UpdateTree(self):
        ...
        files = []# (path, type)
        folders = []# (path, type)
        stack = [self.Particle.config.get("projectPath")]
        while len(stack) > 0:
            path = stack.pop()
        ...
        #root
        self.projectTree.insert("", 0, self.Particle.config.get("projectPath"), text=os.path.basename(self.Particle.config.get("projectPath")), values=(self.Particle.config.get("projectPath"), "dir"), tags=("dir"), image=self.folder_image)
        #folders
        for i in folders:
